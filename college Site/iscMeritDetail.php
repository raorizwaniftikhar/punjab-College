...
<div id="content">
		...
		<?php require_once('includes/sidebar.php');?>	
			<?php
					...
					$getDate="SELECT * FROM admission where prog_id='p3'";
					$result=mysql_query($getDate) or die();
					$currentDate = strtotime(date('Y-m-d'));
					?>
                    <table cellpadding="5" width="50%" style="border:1px solid #FFF; margin:10px;">
                    <tr>
                    	<th bgcolor="#666" colspan="2" style="border-bottom:1px solid #FFF">Merit List for I.C.S</th>
                    </tr>
                    <?php
						while($row=mysql_fetch_array($result))
						{
							if ($currentDate>=strtotime($row['date']))
							{
								?>
                                    ...
                                    <td align="center">
                                    	<a href="icsMerit.php?list_id=<?php echo $status[$row['list_category']]?>">show list</a>
                                    </td>
                            		</tr>
                            <?php
							}
